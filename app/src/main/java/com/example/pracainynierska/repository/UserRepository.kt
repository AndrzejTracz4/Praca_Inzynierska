...
class UserRepository(
    ...
    // Zwraca obiekt User, jeśli użytkownik zostanie znaleziony, w przeciwnym razie zwraca null
    fun getUserByUsernameLiveData(username: String): LiveData<User> = userDao.getUserByUsernameLiveData(username)

    fun getUserByUserUUIDLiveData(userUUID: String): LiveData<User> = userDao.getUserByUUIDLiveData(userUUID)
    ...
    suspend fun updateUserPhotoPath(
        userId: Int?,
        photoPath: String,
    ) {
